...
describe('encoding()', () => {
    ...
    it('parses header with preferences (server priority)', () => {
        expect(Accept.encoding('gzip, identity, deflate', ['deflate', 'gzip', 'identity'])).toBe('deflate');
        expect(Accept.encoding('gzip, identity, deflate', ['gzip', 'identity'])).toBe('gzip');
        expect(Accept.encoding('gzip, identity, deflate;q=0.9', ['deflate', 'gzip', 'identity'])).toBe('gzip');
    });
    ...
    it('errors on invalid header', () => {
        expect(() => Accept.encoding('a;b')).toThrow();
        expect(() => Accept.encoding('a;b;q=1')).toThrow();
        expect(() => Accept.encoding('a;q')).toThrow();
        expect(() => Accept.encoding('a;q=')).toThrow();
        expect(() => Accept.encoding(';q=1')).toThrow();
        expect(() => Accept.encoding('gzip;a=1')).toThrow();
    });
