...
describe('language()', () => {
    ...
    it('returns preference with highest order when equal weigths', () => {
        expect(Accept.language('da, en, en-GB', ['en', 'en-GB'])).toBe('en');
        expect(Accept.language('da, en, en-GB', ['en-GB', 'en'])).toBe('en-GB');
        expect(Accept.language('en, en-GB, en-US')).toBe('en');
    });
    ...
    it('returns language using range match', () => {
        expect(Accept.language('da', ['da-DK'])).toBe('da-DK');
        expect(Accept.language('en-US, en', ['en-GB', 'en-US'])).toBe('en-GB');
        expect(Accept.language('da, en', ['da-DK', 'en-GB'])).toBe('da-DK');
        expect(Accept.language('en, da', ['da-DK', 'en-GB'])).toBe('da-DK');
        expect(Accept.language('en, da', ['en', 'en-GB'])).toBe('en');
        expect(Accept.language('da, en-GB', ['da-DK', 'en-GB'])).toBe('da-DK');
        expect(Accept.language('en, en-GB', ['en-US', 'en-GB', 'da-DK'])).toBe('en-US');
    });

    it('explicit preference overrides range match', () => {
        expect(Accept.language('da, en-GB', ['da-DK', 'en-GB', 'da'])).toBe('en-GB');
    });
});
...
describe('languages()', () => {
    it('parses the header', () => {
        ...
        expect(languages).toEqual(['da', 'en-gb', 'en']);
    });
    ...
    it('ignores invalid weights', () => {
        ...
        expect(languages).toEqual(['da', 'en-gb', 'es', 'en']);
    });
