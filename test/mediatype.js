...
describe('mediaType()', () => {
    ...
    it('prioritizes q value over wildcard', () => {
        expect(Accept.mediaType('text/plain;q=0.1, image/*;q=1', ['text/plain', 'image/jpeg'])).toBe('image/jpeg');
    });

    it('removes extension parameters', () => {
        expect(Accept.mediaTypes('text/html;charset="ascii";q=0.5;ext=123, text/plain;other="xy z";Q=1')).toEqual([
    ...
    it('errors on invalid params', () => {
        expect(() => Accept.mediaType('text/plain;q')).toThrow();
        expect(() => Accept.mediaType('text/plain;q=')).toThrow();
    });
});
...
describe('mediaTypes()', () => {
    it('returns */* when header is missing', () => {
        expect(Accept.mediaTypes()).toEqual(['*/*']);
    });

    it('parses header', () => {
        const mediaTypes = Accept.mediaTypes('text/plain, application/json;q=0.5, text/html, */*;q=0.1, audio/*');
        expect(mediaTypes).toEqual(['audio/*', 'text/html', 'text/plain', 'application/json', '*/*']);
    });

    it('parses header with preferences', () => {
        const mediaTypes = Accept.mediaTypes('text/plain, application/json;q=0.5, text/html, audio/*', [
            'text/*',
            'application/*',
        ]);
        expect(mediaTypes).toEqual(['text/html', 'text/plain', 'application/json']);
    });
    ...
    it('orders by weight', () => {
        ...
        expect(mediaTypes).toEqual(['text/html', 'application/json']);
    });

    it('orders most specific to least specific', () => {
        const types =
            'text/*, text/plain;format=flowed, text/plain, text/plain;level=1, text/html, text/plain;level=2, */*, image/*, text/rich';
    ...
    it('match subtype to preference', () => {
        expect(Accept.mediaTypes('text/html', ['TEXT/*'])).toEqual(['text/html']);
    });

    it('keeps wildcard behind more specific', () => {
        expect(Accept.mediaTypes('text/html, text/*')).toEqual(['text/html', 'text/*']);
        expect(Accept.mediaTypes('text/*, text/html')).toEqual(['text/html', 'text/*']);
    });

    it('matches */* preference to anything', () => {
        expect(Accept.mediaTypes('text/html, text/*, */*', ['*/*'])).toEqual(['text/html', 'text/*', '*/*']);
        expect(Accept.mediaTypes('text/html, text/*, */*', ['*/*', 'TEXT/html'])).toEqual([
            'TEXT/html',
            'text/*',
            '*/*',
        ]);
    ...
    it('invalid weight is ignored', () => {
        ...
        expect(mediaTypes).toEqual(['text/csv', 'text/html', 'text/plain']);
    });

    it('errors on invalid preference', () => {
        expect(() => Accept.mediaTypes('text/html', ['*/html'])).toThrow(
